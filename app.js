...
function ritmo(){
 ...
 function render(){
  content.innerHTML='<div class="card rhythm-card"><div class="game-head"><h2>🥁 El Ritmo del Caribe</h2><span class="badge">Ronda '+(round+1)+'/5</span></div><p>Primero escucha. El ritmo no muestra qué instrumento suena. Después repítelo usando los botones.</p><div class="listen-box" id="listenBox"><div class="listen-wave">〰︎ 〰︎ 〰︎</div><b id="listenText">Pulsa ESCUCHAR RITMO</b><small>Solo escucha y memoriza.</small></div><h3>Escucha cada instrumento</h3><div class="instrument-list">'+ins.map((x,i)=>'<button class="instrument-audio" data-v="'+i+'"><span>'+faces[i]+'</span><b>'+labels[i]+'</b><small>▶ Escuchar</small></button>').join("")+'</div><div class="rhythm-pad">'+ins.map((x,i)=>'<button class="rhythm-btn" data-v="'+i+'" disabled><span>'+faces[i]+'</span><small>'+labels[i]+'</small></button>').join("")+'</div><div class="action-row"><button class="primary big-action" id="startRhythm">🔊 ESCUCHAR RITMO</button></div><div class="feedback" id="fb">Primero escucha el patrón.</div></div>';
...
function pesca(){
 let caught=0,score=0,playing=true,cast=false,fish=[],sp;
 content.innerHTML='<div class="card fishing-card"><div class="game-head"><h2>🎣 Pesca Caribeña</h2><span class="badge"><b id="caught">0</b>/8 peces</span></div><p>El hilo y el cebo siguen tu dedo. Muévelo por el agua y tira del hilo cuando un pez pase cerca del cebo.</p><div class="fishing-zone" id="water"><div class="boat">🛶</div><div class="rod">╲</div><div class="line" id="line"></div><div class="bait" id="bait">🪱</div><div class="cast-guide" id="castGuide">Pulsa «Lanzar hilo»</div></div><div class="fish-controls"><button class="primary" id="cast">🎣 Lanzar hilo</button><button class="primary" id="reel" disabled>⬆️ Tirar del hilo</button></div><div class="stat-line"><span>⭐ Puntos: <b id="fishScore">0</b></span><span id="fishHint">Primero lanza el hilo.</span></div></div>';
 const water=$("#water"),bait=$("#bait"),line=$("#line"),guide=$("#castGuide");
 function move(e){const r=water.getBoundingClientRect(),x=Math.max(8,Math.min(92,(e.clientX-r.left)/r.width*100)),y=Math.max(25,Math.min(88,(e.clientY-r.top)/r.height*100));bait.style.left=x+"%";bait.style.top=y+"%";const bx=r.left+r.width*x/100,by=r.top+r.height*y/100,rx=r.left+r.width*.5,ry=r.top+r.height*.09,dx=bx-rx,dy=by-ry,len=Math.hypot(dx,dy);line.style.width=len+"px";line.style.height="2px";line.style.left="50%";line.style.top="9%";line.style.transform="rotate("+Math.atan2(dy,dx)+"rad)";line.style.transformOrigin="0 50%"}
 water.addEventListener("pointerdown",e=>{if(cast){water.setPointerCapture(e.pointerId);move(e)}});water.addEventListener("pointermove",e=>{if(cast)move(e)});
 $("#cast").onclick=()=>{cast=true;guide.textContent="Mueve el dedo para llevar el cebo";guide.classList.add("hidden");$("#cast").disabled=true;$("#reel").disabled=false;$("#fishHint").textContent="Acerca el cebo a un pez y tira del hilo."};
 $("#reel").onclick=()=>{const b=bait.getBoundingClientRect(),target=fish.find(f=>{const r=f.getBoundingClientRect();return Math.hypot((r.left+r.width/2)-(b.left+b.width/2),(r.top+r.height/2)-(b.top+b.height/2))<65});if(target){target.remove();fish=fish.filter(x=>x!==target);caught++;score+=70;$("#caught").textContent=caught;$("#fishScore").textContent=score;$("#fishHint").textContent="🐟 ¡Pez atrapado!";if(caught>=8)finish()}else{score=Math.max(0,score-10);$("#fishScore").textContent=score;$("#fishHint").textContent="El cebo no estaba lo bastante cerca."}};
 ...
 for(let i=0;i<5;i++)spawn();sp=setInterval(spawn,1200);
